...
export interface AvroSourceData {
  config: NodeConfig;
  onConfigChange: (config: NodeConfig) => void;
}

function AvroSourceNode({ data, selected }: NodeProps<AvroSourceData>) {
  const isConfigured = !!data.config.path;

  const handleBrowse = useCallback(async () => {
    const file = await open({
      multiple: false,
      filters: [{ name: "Avro", extensions: ["avro"] }],
    });
  ...
  return (
    <div className={`rf-node node-source ${selected ? "selected" : ""}`} style={{ minWidth: 220 }}>
      <div className="rf-node-header source">
        <span className="rf-node-title">Avro Source</span>
        ...
        <div className="node-field">
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <input
              ...
              placeholder="Select an Avro file…"
...
export default memo(AvroSourceNode);
